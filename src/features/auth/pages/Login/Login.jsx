...
export default function Login() {
  ...
  const Notification = async (e) => {
    ...
    // Kiểm tra nếu user có role admin thì không cho phép đăng nhập ở trang user
    // Không tiết lộ đó là tài khoản admin để bảo mật
    const isAdmin = user.roles && user.roles.includes("ROLE_ADMIN");
    
    if (isAdmin) {
      // Thông báo lỗi chung chung, không tiết lộ đó là tài khoản admin
      alert("Sai tài khoản hoặc mật khẩu!");
      return;
    }

    // Chỉ cho phép user thông thường đăng nhập ở trang này
    dispatch(updateInfo({
      token: accessToken,
      profile: user
    }));
      ...
      <div className="loginForm">
        <div className="credentialForm">
          ...
          <div className="switchMode">
            <button className="signIn active">Đăng nhập</button>
            <button onClick={redirectTo} className="signUp">
              Đăng ký
            </button>
          </div>
